...
public class AdvisorDialogUtil
{
	...
	{
		try {
			SearchPattern pattern = SearchPattern.createPattern("org.jboss.aop.advice.Interceptor", IJavaSearchConstants.TYPE, IJavaSearchConstants.IMPLEMENTORS, SearchPattern.R_EXACT_MATCH);
			IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new IJavaElement[]{AopCorePlugin.getCurrentJavaProject()},
					IJavaSearchScope.SOURCES | IJavaSearchScope.APPLICATION_LIBRARIES | 
					IJavaSearchScope.REFERENCED_PROJECTS);
			SearchEngine engine = new SearchEngine();
			
			availableInterceptors.clear();
			engine.search(pattern, new SearchParticipant[]{ SearchEngine.getDefaultSearchParticipant() }, scope, new Requestor(shell), new NullProgressMonitor());
		} catch (Exception e)
		{
			e.printStackTrace();
		}
	...
	private static class Requestor extends SearchRequestor
	{
			...
			throws CoreException
		{
			if (match.getElement() instanceof IType)
			{
				...
				if (! Flags.isAbstract(type.getFlags()) && ! type.getPackageFragment().getElementName().startsWith("org.jboss.aop"))
				{
	...
	public static ArrayList getAvaiableAdvice (IJavaElement element, Shell shell, IProgressMonitor monitor)
	{
		...
		SearchPattern pattern = SearchPattern.createPattern("*(*Invocation) Object", IJavaSearchConstants.METHOD, IJavaSearchConstants.DECLARATIONS, SearchPattern.R_PATTERN_MATCH);	
		
		IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new IJavaElement[]{element.getJavaProject()},
				IJavaSearchScope.SOURCES | IJavaSearchScope.APPLICATION_LIBRARIES);
		SearchEngine engine = new SearchEngine();
	...
	private static class AdviceRequestor extends SearchRequestor
	{
			...
			throws CoreException
		{
			if (match.getElement() instanceof IMethod)
			{
				...
				if (PointcuttableValidator.validateMethod(method)
					&& AspectValidator.validateMethod(method))
				{
					if (forElement != null)
					{
		...
		{
			monitor.setTaskName("Found " + method.getDeclaringType().getFullyQualifiedName() + "." + method.getElementName() + "...");

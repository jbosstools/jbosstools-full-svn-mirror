 ...
 * @author Marshall
 */
public class ConvertToPointcutWizard extends Wizard {

	private IJavaElement fromElement;
	private BoundPointcut pointcut;
	private Page page;
	
	public ConvertToPointcutWizard (IJavaElement fromElement)
	{
	...
	private class Page extends WizardPage {
		...
		public void createControl(Composite parent) {
			...
			Font header = new Font(parent.getDisplay(), parent.getFont().getFontData()[0].getName(), parent.getFont().getFontData()[0].getHeight() + 2, SWT.BOLD);
			...
			calledByMethodBrowse.addSelectionListener(new SelectionListener(){
				public void widgetSelected(SelectionEvent e) {
					...
					if (dialog.open() == Window.OK) {
						MethodSelectionDialog methodDialog = new MethodSelectionDialog(getShell(), (IType) dialog.getFirstResult());
						...
						if (methodDialog.open() == Window.OK) {
							...
							calledByMethodText.setText(method.getDeclaringType().getFullyQualifiedName() + "." + method.getElementName());
						}
		...
		private void createFieldControls ()
		{
			...
			get.setText("Get"); set.setText("Set"); getAndSet.setText("Get && Set");
		...
		private class ExtraOptionListener implements SelectionListener {
			public void widgetSelected(SelectionEvent e) {
				...
				else if (fromElement.getElementType() == IJavaElement.METHOD)
				{
					...
					{
						pointcut.setOption(BoundPointcut.USE_METHOD_CALLED_BY_METHOD, new Boolean(calledByMethod.getSelection()));

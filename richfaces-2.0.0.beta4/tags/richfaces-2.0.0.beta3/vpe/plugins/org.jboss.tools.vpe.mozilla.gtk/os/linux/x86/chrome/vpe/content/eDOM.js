/* ***** BEGIN LICENSE BLOCK ***** * Licensed under Version: MPL 1.1/GPL 2.0/LGPL 2.1
...
InsertionPoint.prototype.__defineGetter__(
	...
	function()
	{
		var paraList = getEqualParaList(this);
		var parentForLastElement = null;
		var offsetForLastElement = 0;
		if (this.ipNode.nodeType != Node.TEXT_NODE && this.ipNode.childNodes.length <= this.ipOffset && this.ipNode.childNodes.length > 0) {
			parentForLastElement = this.ipNode;
			offsetForLastElement = this.ipNode.childNodes.length - 1;
		}
		var line = this.line;
		var ipForCount = line.firstInsertionPoint;
		var ipFirst = ipForCount.clone();
		var count = 0;
		while(!ipForCount.equivalent(this))
		{
			for (var i = 0; i < paraList.length; i++) {
				if (paraList[i].equalIP(ipForCount)) {
					return count;
				}
			}
			count++;
			ipForCount.forwardOne();
			if (ipForCount.equivalent(ipFirst)) return 0;
			if (parentForLastElement != null && parentForLastElement == ipForCount.ipNode && offsetForLastElement == ipForCount.ipOffset) {
				break;
			}
		}
		return count;
...
// Edward
///////////////////////////////////////////////////////////////////
function Para(node, offset) {
	this.__node = node;
	this.__offset = offset;
}

Para.prototype.__defineGetter__(
	"node",
	function() {return this.__node;}
);

Para.prototype.__defineGetter__(
	"offset",
	function() {return this.__offset;}
);

Para.prototype.equalIP = function(ip) {
	return ip.ipNode == this.__node && ip.ipOffset == this.__offset;
}

function getEqualParaList(ip) {
	var list = new Array();
	list.push(new Para(ip.ipNode, ip.ipOffset));

	addUpLeft(ip.ipNode, ip.ipOffset, list);
	addUpRight(ip.ipNode, ip.ipOffset, list);
	addDownLeft(ip.ipNode, ip.ipOffset, list);
	addDownLeft(ip.ipNode, ip.ipOffset, list);
	addDownRight(ip.ipNode, ip.ipOffset, list);

	return list;
}

function addUpLeft(node, offset, list) {
	if (offset == 0) {
		offset = node.offset;
		node = node.parentNode;
		if (node != null) {
			list.push(new Para(node, offset));
			addUpLeft(node, offset, list);
			addDownLeft(node, offset, list);
		}
	}
}

function addUpRight(node, offset, list) {
	var length = node.nodeType == Node.TEXT_NODE ? node.nodeValue.length : node.childNodes.length;
	if (offset >= length) {
		var offset = node.offset + 1;
		var node = node.parentNode;
		if (node != null) {
			list.push(new Para(node, offset));
			addUpRight(node, offset, list);
			addUpRight(node, offset, list);
		}
	}
}

function addDownLeft(node, offset, list) {
	if (node.nodeType == Node.ELEMENT_NODE && offset > 0 && node.childNodes.length > 0) {
...
function addDownRight(node, offset, list) {
	if (node.nodeType == Node.ELEMENT_NODE && offset > 0 && node.childNodes.length > 0) {
		if (offset > node.childNodes.length) {
			offset = node.childNodes.length;
		}
		...
		offset = node.nodeType == Node.TEXT_NODE ? node.nodeValue.length : node.childNodes.length;
		list.push(new Para(node, offset));
		addDownRight(node, offset, list);
	}
}

// for test
function ipToString(ip) {
	return pointToString(ip.ipNode, ip.ipOffset);
}

function pointToString(node, offset) {
	if (node.nodeType == Node.TEXT_NODE) {
		return "#text: " + node.nodeValue + "  offset: " + offset;
	} else if (node.nodeType == Node.ELEMENT_NODE) {
		if (node.childNodes.length > offset) {
			return "before: " + node.childNodes[offset].nodeName + "  parent: " + node.nodeName + "  offset: " + offset;
		} else if (node.childNodes.length > 0) {
			return "after: " + node.childNodes[node.childNodes.length - 1].nodeName + "  parent: " + node.nodeName + "  length: " + node.childNodes.length + "  offset: " + offset;
		} else {
			return "into: " + node.nodeName;
		}
...
function paraListToString (list) {
	for (var i = 0; i < list.length; i++) {
		alert("Para " + i + " : " + pointToString(list[i].node, list[i].offset));
	}
}

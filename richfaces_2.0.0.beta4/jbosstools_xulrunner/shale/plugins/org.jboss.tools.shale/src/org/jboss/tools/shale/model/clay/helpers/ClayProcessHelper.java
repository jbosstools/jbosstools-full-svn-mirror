...
public class ClayProcessHelper implements ShaleClayConstants {
	private XModelObject process;
	private XModelObject config;
	private Map<String,XModelObject> items = new HashMap<String,XModelObject>();
	private Map<String,Object> targets = new HashMap<String,Object>();
	XModelObject[] components;
	
	public ClayProcessHelper(ClayProcessImpl process) {
		this.process = process;
	}

	public static ClayProcessHelper getHelper(XModelObject process) {
		return ((ClayProcessImpl)process).getHelper();
	}
	...
	public void restoreRefs() {
		((ClayProcessImpl)process).setReference(process.getParent());
	}
	...
	private void updateProcess0() {
		reset();
		components = config.getChildren(COMPONENT_ENTITY);
		for (int i = 0; i < components.length; i++) {
			String name = components[i].getAttributeValue(ATT_JSFID);
			XModelObject g = findOrCreateItem(name, name);
			items.put(name, g);
			String ext = components[i].getAttributeValue(ATT_EXTENDS);
			if(ext.length() > 0) {
				targets.put(ext, ext);
			}
		}
	...
	private void updateItems() {
		ClayProcessItemImpl[] gs = (ClayProcessItemImpl[])items.values().toArray(new ClayProcessItemImpl[0]);
		for (int i = 0; i < gs.length; i++) {
			setItemReference(gs[i]);
			updateItem(gs[i]);
		}
		gs = (ClayProcessItemImpl[])targets.values().toArray(new ClayProcessItemImpl[0]);
		for (int i = 0; i < gs.length; i++) {
	...
	private void setItemReference(ClayProcessItemImpl item) {
		String path = item.getPathPart();
		XModelObject r = config.getChildByPath(path);
		item.setReference(r);
	}
	
	private void updateItem(ClayProcessItemImpl item) {
		if(!item.isUpToDate()) {
			item.notifyUpdate();
			XModelObject r = item.getReference();
			if(r != null) {
				item.setAttributeValue(ATT_PATH, r.getAttributeValue(ATT_JSFID));
			}
		}
		updateOutputs(item);
	}
	
	private void updateOutputs(ClayProcessItemImpl item) {
		XModelObject r = item.getReference();
		String ext = (r == null) ? "" : r.getAttributeValue("extends");
		updateOutputs(item, ext, item.getChildren());
	}

	private void updateOutputs(ClayProcessItemImpl item, String ext, XModelObject[] outputs) {
		...
		ClayProcessItemOutputImpl r = (ClayProcessItemOutputImpl)output;
	...
	private void updateOutput(ClayProcessItemOutputImpl output) {
///		if(output.isUpToDate()) return;
	...
	private void updateTargets0() {
		Map<String,XModelObject> ds = ClayComponentSet.getInstance(process.getModel()).getDefinitions();

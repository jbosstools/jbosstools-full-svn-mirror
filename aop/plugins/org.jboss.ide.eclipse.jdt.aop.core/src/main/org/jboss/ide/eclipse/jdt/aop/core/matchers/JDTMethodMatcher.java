...
public class JDTMethodMatcher extends MatcherHelper
{
   ...
   public Boolean matches(ASTMethod node)
   {
      try
      {
         ...
         String fqReturnType = JavaModelUtil.getResolvedTypeName(jdtMethod.getReturnType(), jdtMethod
               .getDeclaringType());
         ...
         if (returnType == null)
         {
            ...
            if (!JDTPointcutUtil.matchesClassExprPrimitive(node.getReturnType(), fqReturnType))
               return Boolean.FALSE;
         }
         else
         {
            if (!JDTPointcutUtil.matchesClassExpr(node.getReturnType(), returnType))
               return Boolean.FALSE;
         }

         if (!JDTPointcutUtil.matchesClassExpr(node.getClazz(), jdtMethod.getDeclaringType()))
            return Boolean.FALSE;
         ...
         if (node.getMethodIdentifier().isAnnotation())
         {
            ...
            else
            {
               ...
               {
                  if (node.getMethodIdentifier().getOriginal().equals(tags[k].getName()))
                     return Boolean.TRUE;
               }
               return Boolean.FALSE;
            }
         ...
         {
            if (!node.getMethodIdentifier().matches(jdtMethod.getElementName()))
               return Boolean.FALSE;
         }
         ...
         if (node.getParameters().size() != jdtMethod.getParameterNames().length)
            return Boolean.FALSE;
   ...
   public Object visit(ASTAll node, Object data)
   {
      ...
      {
         if (!JDTPointcutUtil.subtypeOf(jdtMethod.getDeclaringType(), node.getClazz()))
            return Boolean.FALSE;
      }
      else if (node.getClazz().isTypedef())
      {
         if (!JDTPointcutUtil.matchesTypedef(jdtMethod.getDeclaringType(), node.getClazz()))
            return Boolean.FALSE;
      }

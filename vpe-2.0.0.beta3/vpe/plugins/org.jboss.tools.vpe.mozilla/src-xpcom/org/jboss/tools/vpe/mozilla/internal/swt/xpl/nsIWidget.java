...
public class nsIWidget extends nsISupports {
	...
	public Point getBorderSize() {
		...
		int rc = GetBorderSize(aWidth, aHeight);
		if (rc != XPCOM.NS_OK) error(rc);
		return new Point(aWidth[0], aHeight[0]);
	}

	public Rectangle getBounds() {
		int ptr = XPCOM.PR_Malloc(16);
		int rc = GetBounds(ptr);
		if (rc != XPCOM.NS_OK) error(rc);
		int[] buffer = new int[4];
		XPCOM.memmove(buffer, ptr, 16);
		XPCOM.PR_Free(ptr);
		return new Rectangle(buffer[0], buffer[1], buffer[2], buffer[3]);
	}

	public Rectangle getClientBounds() {
		int ptr = XPCOM.PR_Malloc(16);
		int rc = GetClientBounds(ptr);
		if (rc != XPCOM.NS_OK) error(rc);
		int[] buffer = new int[4];
		XPCOM.memmove(buffer, ptr, 16);
		XPCOM.PR_Free(ptr);
	...
	public nsIEnumerator getChildren() {
		int ptr = GetChildren();
		if (ptr != 0) {
			return new nsIEnumerator(ptr);
		} else {
			return null;
		}
	}

...
public class Pages {
	...
	Vector<Object> mPages;
	Vector<Object> sPages = null;
	...
	void createPages() {
		...
		this.mPages = new Vector<Object>();
		this.sPages = new Vector<Object>();
		...
		if (oldCountX != countX || oldCountY != countY) {
			...
			if (oldPages != null) {
				for (int i = 0; i < oldPages.size(); i++) {
					((Page) sPages.get(i)).setSelected(((Page) oldPages.get(i))
							.isSelected());
				}
			} else {
				selectAll();

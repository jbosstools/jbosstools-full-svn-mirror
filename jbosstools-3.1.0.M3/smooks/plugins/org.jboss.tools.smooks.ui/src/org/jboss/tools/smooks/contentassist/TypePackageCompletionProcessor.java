 ...
 * 
 */
public class TypePackageCompletionProcessor implements IContentAssistProcessor {

	private String fErrorMessage;
	private SearchEngine fSearchEngine;
	private Comparator<? super Object> fComparator;
	...
	protected void generateTypePackageProposals(String currentContent, IProject project, Collection<Object> c,
	...
	private void generateProposals(String currentContent, IProject project, final Collection<Object> c,
		...
		class TypePackageCompletionRequestor extends CompletionRequestor {
			...
			public void accept(CompletionProposal proposal) {
				if (proposal.getKind() == CompletionProposal.PACKAGE_REF) {
					...
					addProposalToCollection(c, startOffset, length, pkgName, pkgName, SmooksConfigurationActivator
							.getDefault().getImageRegistry().get(GraphicsConstants.IMAGE_PACKAGE_OBJECT));
				} else {
					...
					Image image = isInterface ? SmooksConfigurationActivator.getDefault().getImageRegistry().get(
							GraphicsConstants.IMAGE_JAVA_INTERFACE) : SmooksConfigurationActivator.getDefault()
							.getImageRegistry().get(GraphicsConstants.IMAGE_JAVA_OBJECT);
	...
	protected void generateTypeProposals(String currentContent, IProject project, final Collection<Object> c,
			final int startOffset, final int length, int typeScope) {
		// Dynamically adjust the search scope depending on the current
		// state of the project
		IJavaSearchScope scope = SmooksUIUtils.getSearchScope(project);
		...
		try {
			TypeNameRequestor req = new TypeNameRequestor() {
				public void acceptType(int modifiers, char[] packageName, char[] simpleTypeName,
					...
					Image image = (Flags.isInterface(modifiers)) ? SmooksConfigurationActivator.getDefault()
							.getImageRegistry().get(GraphicsConstants.IMAGE_JAVA_INTERFACE)
							: SmooksConfigurationActivator.getDefault().getImageRegistry().get(
	 ...
	 * @param image
	 */
	protected void addProposalToCollection(final Collection<Object> c, final int startOffset, final int length,
			String label, String content, Image image) {

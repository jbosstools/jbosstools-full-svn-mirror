 ...
 *     Red Hat, Inc. 
 *******************************************************************************/
package org.jboss.tools.vpe.editor.menu.xpl;
...
public class ActionHelper {
	...
	protected static class Action implements ModelQueryAction {
		...
		public Action(int kind, Node parent, CMNode cmNode, int startIndex,
				int endIndex) {
			this.kind = kind;
			this.parent = parent;
			this.cmNode = cmNode;
	...
	public ActionHelper(ModelQueryImpl modelQuery) {
    ...
    if ((includeOptions & ModelQuery.INCLUDE_ATTRIBUTES) != 0) {
		...
		if ((includeOptions & ModelQuery.INCLUDE_CHILD_NODES) != 0) {
			if (index != -1) {
				getInsertChildNodeActionsAtIndex(parent, ed, index,
						includeOptions, validityChecking, actionList);
			} else {
				getInsertChildNodeActions(parent, ed, includeOptions,
						validityChecking, actionList);
			}
		}
	}

	protected void getInsertAttributeActions(Element parent,
			CMElementDeclaration ed, int validityChecking, List actionList) {
		// get actions for each insertable attribute
		List availableAttributeList = modelQuery.getAvailableContent(parent,
				ed, ModelQuery.INCLUDE_ATTRIBUTES);
		...
		for (Iterator i = availableAttributeList.iterator(); i.hasNext();) {
			...
			if (modelQuery.canInsert(parent, ed, ad, 0, validityChecking)) {
				Action action = new Action(Action.INSERT, parent, ad);
				actionList.add(action);
			}
		}
	}

	protected void getInsertChildNodeActionsAtIndex(Element parent,
		...
		for (Iterator i = availableChildNodeList.iterator(); i.hasNext();) {
			...
			if (isSimpleChoice) {
				Action action = new Action(Action.INSERT, parent, cmNode,
						index, index);
				actionList.add(action);
			}
		}
	...
	protected boolean isSimpleChoiceGroupContentModel(CMElementDeclaration ed) {       
		...
		if (cmNode != null && cmNode.getNodeType() == CMNode.GROUP) {
			...
			if (cmGroup.getOperator() == CMGroup.CHOICE
					&& cmGroup.getMaxOccur() == -1) {
				result = true;
	...
	protected void getInsertChildNodeActions(Element parent,
		...
		for (Iterator iterator = availableChildNodeList.iterator(); iterator
				.hasNext();) {
	...
	public void getInsertActions(Document parent, CMDocument cmDocument,
			int index, int includeOptions, int validityChecking, List actionList) {
		...
		// make sure that root elements are only added after the doctype (if
		// any)
		if (rootElement == null && index > doctypeIndex) {
			...
			for (int i = 0; i < mapLength; i++) {
				...
				if (validityChecking == ModelQuery.VALIDITY_STRICT) {
					canAdd = doctype == null
							|| doctype.getName().equals(cmNode.getNodeName());
				}
	...
	public void getReplaceActions(Element parent, CMElementDeclaration ed,
			int includeOptions, int validityChecking, List actionList) {
		CMValidator.MatchModelNode matchModelNode = modelQuery.getValidator()
				.getMatchModel(ed, parent);
		if (matchModelNode != null) {
			MatchModelVisitor visitor = new MatchModelVisitor(parent,
					actionList);
	...
	public void getReplaceActions(Element parent, CMElementDeclaration ed,
		...
		if (selectedChildren.size() == 1) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				CMNode childEd = modelQuery
						.getCMElementDeclaration(childElement);
				if (childEd != null) {

					CMNode childOrigin = (CMNode) modelQuery
	...
	protected boolean isContiguous(Element parent, int[] range,
	...
	protected boolean isActionContainingRange(ModelQueryAction action,
	...
	protected boolean isActionMatchingRange(ModelQueryAction action,
			int startIndex, int endIndex) {
		int actionStartIndex = action.getStartIndex();
		int actionEndIndex = action.getEndIndex();
		return (actionStartIndex == startIndex && actionEndIndex == endIndex);
	...
	protected void removeActionsNotContainingRange(List actionList,
			int startIndex, int endIndex) {
	...
	protected void removeActionsNotMatchingRange(List actionList,
			int startIndex, int endIndex) {
		for (int i = actionList.size() - 1; i >= 0; i--) {
			ModelQueryAction action = (ModelQueryAction) actionList.get(i);
	...
	public static class MatchModelVisitor {
		...
		public void visitMatchModelNode(
				CMValidator.MatchModelNode matchModelNode) {
			...
			for (Iterator iterator = matchModelNode.children.iterator(); iterator
					.hasNext();) {
				CMValidator.MatchModelNode child = (CMValidator.MatchModelNode) iterator
						.next();
			...
			if (matchModelNode.cmNode != null) {
				...
				if (nodeType == CMNode.GROUP) {
					...
					if (group.getOperator() == CMGroup.CHOICE) {
						addReplaceActions(matchModelNode, group, startIndex,
								elementIndex - 1);
					}
		...
		public void addReplaceActions(
				CMValidator.MatchModelNode matchModelNode, CMGroup group,
				int startIndex, int endIndex) {
			CMNode excludeCMNode = null;
			if (matchModelNode.children.size() > 0) {
				CMValidator.MatchModelNode child = (CMValidator.MatchModelNode) matchModelNode.children
						.get(0);
			...
			for (int i = 0; i < size; i++) {
				...
				if (alternative != excludeCMNode) {
					Action action = new Action(Action.REPLACE, parent,
							alternative, startIndex, endIndex);
					actionList.add(action);
				}
			}
		}

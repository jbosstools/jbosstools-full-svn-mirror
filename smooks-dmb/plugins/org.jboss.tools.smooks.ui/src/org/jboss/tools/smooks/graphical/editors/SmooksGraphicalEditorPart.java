...
public class SmooksGraphicalEditorPart extends GraphicalEditor implements ISelectionChangedListener,
	...
	protected SmooksModel smooksResourceList;
...
//					}
	...
	protected void refershRecentAffectedModel(Command command, Collection<?> affectedObjects, int commandType) {
		for (Iterator<?> iterator = affectedObjects.iterator(); iterator.hasNext();) {
			...
			if (commandType == EXECUTE_COMMAND || commandType == REDO_COMMAND) {
				if (object instanceof SmooksModel) {
					if (command instanceof AddCommand) {
					...
					if (command instanceof AddCommand) {
						...
						if (owner instanceof SmooksModel) {
	...
	protected void initGraphicalModel() {
		...
		AdapterFactoryEditingDomain editingDomain = (AdapterFactoryEditingDomain) smooksModelProvider
				.getEditingDomain();
		if (inputDataList != null && obj != null && obj instanceof Model && editingDomain != null) {
			createInputDataGraphModel();
			SmooksModel listType = getSmooksResourceList();
			List<?> arcList = listType.getComponents();
...
//		this.validateEnd(diagnosticList);
	...
	public SmooksModel getSmooksResourceList() {
		if (smooksModelProvider != null) {
			SmooksModel m = null;
			Model<SmooksModel> smooksModel = smooksModelProvider.getSmooksModel();
			if(smooksModel != null){
			     m = smooksModel.getModelRoot();
			}
			return m;
		}
		return null;
	...
	protected void initSmooksData() {
		smooksResourceList = getSmooksResourceList();
			if(smooksResourceList != null){
			IJavaProject jp = JavaCore.create(((IFileEditorInput)getEditorInput()).getFile().getProject());
			inputDataList = SelectorCreationDialog.generateInputData(smooksResourceList,jp);
		}
	}
...
//				}
	...
	public SmooksModel getSmooksResourceListType() {
		if (smooksModelProvider != null) {
			Object obj = smooksModelProvider.getSmooksModel();
			smooksResourceList = null;
			if (obj instanceof Model) {
				smooksResourceList = ((Model<SmooksModel>) obj).getModelRoot();
			}
	...
	public void inputTypeChanged() {
		if (root != null && inputDataList != null) {
			...
			List<Object> newInputDataList = SelectorCreationDialog.generateInputData(getSmooksResourceListType() , jp);
	...
	protected String getDiagnosticMessage(Diagnostic diagnostic) {
		...
		if (datas.size() == 2) {
			...
			if (parentObj instanceof IBean) {
				Object obj = datas.get(1);
				if (obj == JavaBeanPackage.Literals.BEAN__BEAN_ID) {
			...
			if (parentObj instanceof IValue) {
				Object obj = datas.get(1);
				if (obj == JavaBeanPackage.Literals.VALUE__DATA) {
					String message = diagnostic.getMessage();
					if (message != null && message.startsWith("The required feature")) { //$NON-NLS-1$
			...
			if (parentObj instanceof IWiring) {
				Object obj = datas.get(1);
				if (obj == JavaBeanPackage.Literals.WIRING__BEAN_ID_REF) {
					String message = diagnostic.getMessage();
					if (message != null && message.startsWith("The required feature")) { //$NON-NLS-1$

...
public class CreateJavaBeanModelCommand extends GEFAdapterCommand {
		...
		if (firstTime) {
			...
			if (editorPart != null && collections != null && owner != null && feature != null) {
				...
				if (dialog.open() == Dialog.OK) {
					...
					int type = BEAN_TYPE;
					SmooksModel resourceListType = null;
	...
	private List<Object> createJavaBeanModel(int type, JavaBeanModel parentBeanModel, Object[] properties,
			SmooksModel resourceListType, List<String> ids) {
		List<Object> creationObject = new ArrayList<Object>();
		Object parent = null;
		if (type == BEAN_TYPE) {
			parent = JavaBeanFactory.eINSTANCE.createBean();
		}
		creationObject.add(parent);
		String beanID = generateBeanID(parentBeanModel, resourceListType, ids);
		ids.add(beanID);
		if (parent instanceof IBean) {
			((IBean) parent).setBeanId(beanID);
			((IBean) parent).setBeanClass(parentBeanModel.getBeanClassString());
				...
				for (int i = 0; i < properties.length; i++) {
					...
					if (beanPropertyObj instanceof JavaBeanModel && belongsToMe(parentBeanModel, (JavaBeanModel) beanPropertyObj)) {
						...
						if (beanProperty.isPrimitive()) {
							IValue value = JavaBeanFactory.eINSTANCE.createValue();
							value.setProperty(beanProperty.getName());
							((IBean) parent).getValueBindings().add(value);
						} else {
							IWiring value = JavaBeanFactory.eINSTANCE.createWiring();
							...
							((IBean) parent).getWireBindings().add(value);
...
//		}
	...
	private String generateBeanID(JavaBeanModel parentBeanModel, SmooksModel listType, List<String> ids) {
	...
	private boolean idExsit(String id, SmooksModel listType, List<String> ids) {

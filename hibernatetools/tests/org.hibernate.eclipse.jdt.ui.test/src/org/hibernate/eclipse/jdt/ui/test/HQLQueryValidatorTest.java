...
public class HQLQueryValidatorTest extends HibernateConsoleTest {
	...
	public void testCheckQueryEL() {
		...
		ccfg.checkQuery("from java.lang.Object", false); //$NON-NLS-1$
		ccfg.checkQuery("from TestClass", false); //$NON-NLS-1$
		...
		ccfg.checkQuery("from TestClass where id = #{some.id.field=}", true); //$NON-NLS-1$
		ccfg.checkQuery("from TestClass where id = #{some.id and field=}", true); //$NON-NLS-1$
	}

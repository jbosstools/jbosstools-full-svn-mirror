...
public class PrintPreviewDialog extends Dialog {
	...
	public class ControlPanel extends Composite implements PaintListener {
		...
		public void paintControl(PaintEvent pevent) {
			GC gc = pevent.gc;
			gc.setForeground(new Color(ControlPanel.this.getDisplay(), 0x00,
					0x00, 0x00));
			gc.drawLine(pevent.x, pevent.y, pevent.width, pevent.x);
			gc.drawLine(pevent.x, pevent.y, pevent.x, pevent.height);
			gc.drawLine(pevent.x, pevent.y + pevent.height - 2, pevent.x
					+ pevent.width, pevent.y + pevent.height - 2);
			gc.drawLine(pevent.x + pevent.width - 2, pevent.y + pevent.height,
					pevent.x + pevent.width - 2, pevent.y);
			gc.setForeground(new Color(ControlPanel.this.getDisplay(), 0xff,
					0xff, 0xff));
			gc.drawLine(pevent.x + 1, pevent.y + 1, pevent.width - 1,
					pevent.x + 1);
			gc.drawLine(pevent.x + 1, pevent.y + 1, pevent.x + 1,
					pevent.height - 1);
			gc.dispose();
		}
	...
	public String open() {
		...
		imageView = new PagesView(pages, new Dimension(550, 370), dialog,
				SWT.RESIZE);
		imageView
				.setBackground(new Color(dialog.getDisplay(), 0xff, 0xff, 0xff));
		...
		ok.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (editor.isBordersPaint()) {
					if (!ClayEditor.showMessage(PrintPreviewDialog.this
							.getParent(), 2))
						result = "cancel";
					PrintPreviewDialog.this.pages.unSelectAll();
					PrintPreviewDialog.this.pages.getSourcePage(0).setSelected(
							true);
				}
				dialog.dispose();
			}

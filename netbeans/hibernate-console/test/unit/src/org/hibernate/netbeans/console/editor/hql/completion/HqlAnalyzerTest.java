...
public class HqlAnalyzerTest extends TestCase {
    ...
    public void doTestVisibleSubQueries(String query, int size) {
        List<HqlAnalyzer.SubQuery> visible = HqlAnalyzer.getVisibleSubQueries(query.replaceAll("\\|", "").toCharArray(), query.indexOf("|"));
        assertEquals("Invalid visible query size", size, visible.size());
    }

    private void doTestSubQueries(String query, int size) {
        List l = HqlAnalyzer.getSubQueries(query.toCharArray(), 0).subQueries;
        assertEquals("Incorrent subqueries count", size, l.size());
    }

    private void doTestPrefix(String query, String prefix) {
        assertEquals(prefix, HqlAnalyzer.getTableNamePrefix(query.toCharArray(), query.indexOf("|")));
    }

    private void doTestShouldShowTables(String query, boolean expectedValue) {
        if (expectedValue) {
            assertTrue(HqlAnalyzer.shouldShowTables(query.toCharArray(), getCaretPosition(query)));
        } else {
            assertFalse(HqlAnalyzer.shouldShowTables(query.toCharArray(), getCaretPosition(query)));
        }
    }
    ...
    private void doTestVisibleTables(String query, String[] types, String aliases[]) {
        List<QueryTable> qts = HqlAnalyzer.getVisibleTables(query.replaceAll("\\|", "").toCharArray(), getCaretPosition(query));
        assertEquals("Incorrect table count", types.length, qts.size());
        int i = 0;
        for (QueryTable qt : qts) {
            assertEquals("Incorrect query table type [" + i + "]", types[i], qt.getType());

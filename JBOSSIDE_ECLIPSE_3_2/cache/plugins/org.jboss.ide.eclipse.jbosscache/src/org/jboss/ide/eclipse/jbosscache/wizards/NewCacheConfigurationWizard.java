...
public class NewCacheConfigurationWizard extends Wizard implements INewWizard
{
   ...
   public boolean performFinish()
   {
      try
      {
         ...
         //File file = CacheUtil.createNewFile(cacheConfigModel.getCacheName()+ICacheConstants.XML_FILEEXTENSION);
         final File file = new Path(cacheConfigModel.getDirectoryName() + File.separatorChar + confFileName
               + ICacheConstants.XML_FILEEXTENSION).toFile();
         if (file.exists())
         {
            MessageDialog.openError(getShell(), CacheUtil
                  .getResourceBundleValue(ICacheConstants.CONFIGURATION_ERROR_MESSAGE_CACHEEXIST_TITLE), CacheUtil
         ...
         IRunnableWithProgress prog = new IRunnableWithProgress()
         {
            ...
            {
               monitor.beginTask(JBossCachePlugin.getResourceBundle().getString(
                     ICacheConstants.CONFIGURATION_MONITOR_TASK_NAME), 10);
               try
               {
                  monitor.worked(1);
                  CacheUtil.saveMetaDataInformation(cacheConfigModel, monitor, file);
               }
               catch (Exception e)
               {
         ...
         ICacheRootInstance rootInstance = CacheInstanceFactory.getCacheRootInstance(cacheName, cacheConfigModel
               .getDirectoryName()
          ...
          */
         if (workBench == null)
            workBench = viewPart.getSite().getWorkbenchWindow().getWorkbench();
   ...
   {
      cacheConfigModel.setDirectoryName(stdConfPage.getTxtDirectoryName().getText().trim());
      cacheConfigModel.setCacheName(stdConfPage.getTxtCacheName().getText().trim());
      cacheConfigModel.setCacheMode(stdConfPage.getCmbCacheMode().getText().trim());
      cacheConfigModel.setClusterName(stdConfPage.getTxtClusterName().getText().trim());
      //cacheConfigModel.setEvictionPolicyClass(stdConfPage.getTxtEvictionPolicyClass().getText());
      cacheConfigModel.setFetchStateOnStartup(stdConfPage.getChkFetchStateOnStartup().getSelection());
      cacheConfigModel.setInitialStateRetrievalTimeOut(stdConfPage.getTxtInitialStateRetrievalTimeout().getText()
            .trim());
      cacheConfigModel.setIsolationLevel(stdConfPage.getCmbIsolationLevel().getText().trim());
      cacheConfigModel.setLockAcquisitionTimeOut(stdConfPage.getTxtLockAcquisitionTimeout().getText().trim());
      cacheConfigModel.setReplQueueInterval(stdConfPage.getTxtReplicationQueueInterval().getText().trim());
      cacheConfigModel.setReplQueueMaxElements(stdConfPage.getTxtReplQueueMaxElements().getText().trim());
      cacheConfigModel.setSyncReplicationTimeOut(stdConfPage.getTxtSyncReplTimeout().getText().trim());
      cacheConfigModel.setTransManagerLookUpClass(stdConfPage.getCmbTransactionManager().getText().trim());
      cacheConfigModel.setUseReplQueue(stdConfPage.getChkUseReplicationQueue().getSelection());
   }

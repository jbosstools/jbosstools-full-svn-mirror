...
public abstract class TemplateBuilder {
    ...
    /**
     * Public constructor.
     * @param modelBuilder The model builder underlying this template builder instance.
     ...
     * @return The template.
     * @throws org.jboss.tools.smooks.templating.template.exception.TemplateBuilderException Exception building template.
     */
     ...
     * @throws XPathExpressionException error evaluating XPath expression.
     */
    ...
    /**
     * Get the {@link ModelBuilder} instance associated with this {@link TemplateBuilder}.
     * @return The {@link ModelBuilder} instance associated with this {@link TemplateBuilder}.
     */
    ...
    public List<Node> removeMapping(Mapping mapping) {
    	...
    	for(Node hiddenNode : mapping.getHideNodes()) {
	...
	private void asserValidMappingNode(Node mappingNode) throws InvalidMappingException {
        ...
        if(ModelBuilder.NAMESPACE.equals(mappingNode.getNamespaceURI())) {
            throw new InvalidMappingException("Unsupported XML target node mapping.  Cannot map to a reserved model node from the '" + ModelBuilder.NAMESPACE + "' namespace."); //$NON-NLS-1$
        }
        if(ModelBuilder.isHidden(mappingNode)) {
            throw new InvalidMappingException("Illegal XML target node mapping for node '" + mappingNode + "'.  This node (or one of it's ancestors) is hidden."); //$NON-NLS-1$
        }
    }
	...
	/**
	 * Get the nearest "Collection" element to for the supplied
	 * model node.
	 * <p/>
	 * This can be the node itself, if it is a Collection node.
	 * 
	 * @param modelPath The starting point of the search.
	 * @return The nearest Collection element (possibly itself), or null if there is non.
	 */
	...
	/**
	 * Assert whether or not the specified model node details should be added to the
	 * template.
	 * @param element The model element.
	 * @return True if the model node details should be added to the template, otherwise false.
	 */
	protected boolean assertAddNodeToTemplate(Element element) {
		...
		//   2. Is hidden.
		//   3. Has a minOccurs of 0 and has no mappings onto it (including any of it's attributes and 
	...
	/**
	 * Is the supplied element on the path of any of the currently mapped model nodes.
	 * @param element The element to be checked.
	 * @return True if the element is on the path of one of the Mappings, otherwise false.
	 */
	...
	/**
	 * Add a collection mapping from the supplied template model element
	 * and target {@link ModelNodeResolver}.
	 * @param element The element from the template model (DOM model built from a template).
	 * @param modelNodeResolver The target model node resolver.
	 * @throws TemplateBuilderException Invalid collection element.
	 */

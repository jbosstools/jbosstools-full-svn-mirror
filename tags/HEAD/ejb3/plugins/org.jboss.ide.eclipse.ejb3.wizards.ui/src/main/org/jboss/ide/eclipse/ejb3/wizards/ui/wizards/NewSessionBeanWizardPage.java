...
public class NewSessionBeanWizardPage extends NewTypeWizardPage {
		...
		protected void createBeanTypeControls (Composite composite, int nColumns)
		{
			...
			Label beanTypeLabel = createLabel(composite, EJB3WizardsUIPlugin.getResourceString("NewSessionBeanWizard.beanTypeLabel"));
		...
		{	
			Label sessionPackageNameLabel = createLabel(composite, EJB3WizardsUIPlugin.getResourceString("NewSessionBeanWizard.beanPackageLabel"));
			...
			Label beanNameLabel = createLabel(composite, EJB3WizardsUIPlugin.getResourceString("NewSessionBeanWizard.beanNameLabel"));
			...
			Label sessionClassNameLabel = createLabel(composite, EJB3WizardsUIPlugin.getResourceString("NewSessionBeanWizard.beanClassNameLabel"));
			beanClassName = createLabelWithImage(composite, "", JavaUI.getSharedImages().getImage(ISharedImages.IMG_OBJS_CLASS), nColumns - 1);
		}
		...
		protected void createRemoteInterfaceControls (Composite composite, int nColumns)
		{
			...
			useCustomRemoteInterfacePackageButton.setText(EJB3WizardsUIPlugin.getResourceString("NewSessionBeanWizard.useCustomInterfacePackageButtonLabel"));
			...
			remoteInterfaceNameLabel = createLabelWithImage(composite, "", JavaUI.getSharedImages().getImage(ISharedImages.IMG_OBJS_INTERFACE), nColumns - 1);
		}
...
//			beanType.getCompilationUnit().becomeWorkingCopy()
			...
			importDecl.setName(ast.newName(new String[] { "javax", "ejb", beanType }));
		...
		public ICompilationUnit createRemoteInterface (IProgressMonitor monitor)
			throws CoreException, InterruptedException
		{
			// lame.
			String source =
				"package " + remoteInterfacePackage + ";\n" +
				"\n"+
...
//			System.out.println(ast.toString());
		...
		protected IPackageFragment getPackageFragment (String packageName)
		...
		class TypeNameGetter implements Runnable {
			...
			public TypeNameGetter () { typeName = ""; }
		...
		void updateTypeNames ()
		{
			Display.getDefault().asyncExec(new Runnable() {
				public void run () {
					...
					beanClassName.setText(getClassName(packageName, beanName) + (beanName != null && beanName.length() > 0 ? "Bean" : ""));

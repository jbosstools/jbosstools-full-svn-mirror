 ...
 * @created   18 mai 2003
 */
public class TerminateDefaultServerActionDelegate extends AbstractDefaultServerActionDelegate
{
   ...
   public void run(IAction action)
   {
      try
      {
         ServerLaunchManager.getInstance().terminate(ServerLaunchManager.getInstance().getDefaultLaunchConfiguration());
      }
   ...
   protected void enableAction(ILaunchConfiguration configuration, IAction action)
   {
      try
      {
         ServerLaunchUIUtil.enableServerActions(configuration, null, null, action);
      }
      catch (CoreException e)
      {
         AbstractPlugin.log(e);
